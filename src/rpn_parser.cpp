...
// تحديد إذا كان العامل أيسر أم أيمن
static bool right_associative(char op) {
    return op == '^';
}

// دالة مساعدة: تخطي المسافات
    ...
    while (i < expr.size()) {
        ...
        if (std::isdigit(c) || c == '.') {
            ...
            if (name == "sin") tt = TokenType::SIN;
                ...
                // غير معروف – تجاهل أو خطأ
                continue;
            }
            ops.push(static_cast<char>(tt));   // نستخدم char مؤقتاً
        }
        else {
            // حرف غير معروف – نتخطى
            ++i;
